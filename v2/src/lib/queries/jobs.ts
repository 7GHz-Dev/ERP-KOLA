...
export async function listJobs(filter: JobFilter = {}) {
  ...
  const rows = await db
    .select({
      ...
      portId: jobs.portId,
      // ไอดีของ master ที่หน้าแก้ BL ต้องใช้เติมค่าเดิมลงในช่องเลือก
      consigneeId: jobs.consigneeId,
      notifyPartyId: jobs.notifyPartyId,
      jobTypeId: jobs.jobTypeId,
      shipperId: jobs.shipperId,
      blType: jobs.blType,
      goodsValue: jobs.goodsValue,
      goodsCurrency: jobs.goodsCurrency,
      customerNote: jobs.customerNote,
...
/**
 * คัดงานตามว่ามีไฟล์เอกสารต้นเรื่องแนบอยู่หรือยัง — Arrival Notice หรือ BL
 *
 * `has: false` = ไม่มีทั้งสองหมวด ใช้คัดงานที่นำเข้ามาจากไฟล์ตาราง (CSV/Excel)
 * ซึ่งมีข้อมูลครบแล้วแต่ยังไม่มีไฟล์แนบ จะได้ไล่อัปไฟล์ให้ครบโดยไม่ต้องเปิดดูทีละงาน
 *
 * `has: true` = มีอย่างน้อยหนึ่งหมวด คือฝั่งตรงข้ามกันพอดี
 * งานที่มี AN แล้วแต่ยังขาด BL ถือว่ามีเอกสารต้นเรื่องแล้ว จึงอยู่ฝั่งนี้
 *
 * รวมสองฝั่งไว้ในฟังก์ชันเดียวเพราะต่างกันแค่ exists กับ not exists
 * ถ้าแยกเขียนสองชุด จะมีวันที่แก้หมวดไฟล์ที่ฝั่งหนึ่งแล้วลืมอีกฝั่ง
 * แล้วทั้งสองปุ่มรวมกันได้ไม่ครบทุกงาน ซึ่งดูจากหน้าจอไม่ออกว่าหายไปไหน
 *
 * exists อ่านเร็วเพราะ files มี index (job_id, category, is_current) อยู่แล้ว
 * และหยุดทันทีที่เจอแถวแรก ไม่ได้นับทั้งหมดก่อนค่อยเทียบ
 */
export const hasArrivalFiles = (has: boolean) =>
  sql`${has ? sql`exists` : sql`not exists`} (select 1 from files f

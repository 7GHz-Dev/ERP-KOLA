...
function stripLabel(raw: string): string {
  ...
  /*
   * ตัวคั่นที่ค้างอยู่หน้าค่า
   *
   * บางใบ pdf.js หั่นข้อความโดยเอาเครื่องหมายของป้ายไปไว้กับค่า
   * เช่น EVERGREEN Arrival Notice ที่ได้ ": EGLV060600120711" มาทั้งชิ้น
   * (ป้าย "B/L NO" อยู่คนละชิ้นกัน ตัวตัดป้ายข้างบนจึงไม่เห็น)
   *
   * ตัดเฉพาะที่อยู่หน้าสุด และไม่ตัดขีดกลางเพราะเป็นส่วนหนึ่งของรหัสได้
   */
  v = clean(v.replace(/^[:;.,|]+\s*/, ''));
  return v;
}
...
export function valueFromText(field: TemplateFieldKey, raw: string): string | string[] {
  ...
  switch (field) {
    ...
    case 'seals':
       ...
       *
       * ต้องมีทั้งตัวอักษรและตัวเลขปนกัน — เดิมขอแค่ "มีตัวเลข" ซึ่งหลวมไป
       * กรอบที่กว้างพอจะรับตารางที่เลื่อนได้ ก็กวาดตัวเลขในตารางมาด้วย
       * แล้วได้ซีลปลอมอย่าง "1.0000" กับ "33.9161000" จากคอลัมน์น้ำหนัก/ปริมาตร
       */
      return text.toUpperCase().split(/[\s,;/|]+/)
        ...
        .filter((v) => v.length >= 4 && v.length <= 20
          ...
          && !/^[A-Z]{4}\d{7}$/.test(v)
          // รหัสขนาดตู้ที่วางไว้ระหว่างเลขตู้กับเลขซีล เช่น 40SD96, 20GP, 45HC
          && !/^(20|40|45)[A-Z]{2}\d{0,2}$/.test(v));
